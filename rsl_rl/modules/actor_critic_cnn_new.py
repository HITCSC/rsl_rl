...
class RMAEncoder(nn.Module):
    """Map a flat observation vector to a 3D body-frame velocity command ``[vx, vy, wz]``."""
    VELOCITY_DIM = 3
    def __init__(
        self,
        input_dim: int,
...
class ActorCriticCNNNew(nn.Module):
    is_recurrent = False
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # initialize height map cnn for actor and precise height map cnn for critic.
        self.height_map_cnn = height_map_cnn
        self.precise_height_map_cnn = precise_height_map_cnn
        ...
        # add precise height map cnn output dim
        num_critic_obs_perceptive = num_critic_obs + self.precise_height_map_cnn.output_dim
    ...
    def evaluate(self, obs, **kwargs):
        ...
        # concatenate height map cnn output
        height_map_features = self.precise_height_map_cnn(perceptive.unsqueeze(-1)).squeeze(-1)
        obs_perceptive = torch.cat([obs, height_map_features], dim=-1)
    ...
    def get_critic_obs(self, obs):
        ...
        # hls: 临时修改，这里为了取前半部分高程图，先把perception_obs给critic
        return torch.cat(obs_list, dim=-1), self.get_perception_obs(obs)
