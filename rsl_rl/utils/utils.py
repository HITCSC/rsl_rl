...
def load_optimizer_policy_only(
    ...
    *,
    ...
    """Load Adam optimizer state for policy parameters only."""
    policy_params = list(policy.parameters())
    ckpt_param_groups = checkpoint_optimizer_state.get("param_groups", [])
    ckpt_state = checkpoint_optimizer_state.get("state", {})

    if not ckpt_param_groups:
        raise ValueError("Checkpoint optimizer state does not contain 'param_groups'.")

    ckpt_policy_group = None
    for group in ckpt_param_groups:
        if group.get("name") == "policy" or ckpt_policy_group is None:
            ckpt_policy_group = group
        if group.get("name") == "policy":
            break

    ckpt_param_ids = ckpt_policy_group["params"]
    if len(ckpt_param_ids) != len(policy_params):
        msg = (
            f"Policy parameter count mismatch: checkpoint optimizer has {len(ckpt_param_ids)} params, "
            f"current policy has {len(policy_params)}."
        )
        if strict:
            raise ValueError(msg)
        print(f"[WARNING] {msg} Loading optimizer state for the first {min(len(ckpt_param_ids), len(policy_params))} params.")

    loaded = 0
    for param, ckpt_param_id in zip(policy_params, ckpt_param_ids):
        if ckpt_param_id not in ckpt_state:
            continue
        state = ckpt_state[ckpt_param_id]
        optimizer.state[param] = {
            key: value.to(param.device).clone() if isinstance(value, torch.Tensor) else value
            for key, value in state.items()
        }
        loaded += 1

    if copy_lr:
        ckpt_lr = ckpt_policy_group.get("lr")
    ...
    print(
        f"[INFO] Loaded optimizer state for {loaded}/{len(policy_params)} policy parameters. "
        "Discriminator optimizer states remain freshly initialized."
    )
    return loaded
