...
class DPPO:
    ...
    def __init__(
        ...
        self.rma_loss_coef = rma_loss_coef
    ...
    def process_env_step_distillation(self, obs, rewards, dones, extras, student_flag, half_size):
        # update the normalizers
        self.policy.update_normalization(obs)
        ...
        if ("time_outs" in extras) and  student_flag:
            self.transition.rewards += self.gamma * torch.squeeze(
                self.transition.values * extras["time_outs"].unsqueeze(1).to(self.device), 1
            )
        # record the transition
    ...
    def update_distillation(self):
        ...
        for epoch in range(self.num_learning_epochs):
            ...
            for obs, privileged_actions, dones in self.storage.generator2distillation():

                # inference the student for gradient computation
                actions = self.policy.act_inference(obs)
                if not actions.requires_grad:
                    print("警告：student_output 没有梯度")

                # behavior cloning loss
                behavior_loss = self.loss_fn(actions, privileged_actions.detach())

                # RMA supervision loss (optional)
                rma_loss = self.policy.compute_rma_loss(obs) if self.use_rma else obs["policy"].new_tensor(0.0)

                # total loss
                loss = loss + behavior_loss + self.rma_loss_coef * rma_loss
    ...
    def update_rl_distillation(self, student_flag):  # noqa: C901
        global global_step, total_decay_steps, initial_alpha, min_alpha
        mean_value_loss = 0
        mean_surrogate_loss = 0
        mean_entropy = 0
        mean_behavior_loss = 0
        ...
        if global_step < total_decay_steps:
                # 余弦退火公式（平滑下降）
            progress = global_step / total_decay_steps
            cosine_decay = 0.5 * (1 + math.cos(math.pi * progress))
            alpha = min_alpha + (initial_alpha - min_alpha) * cosine_decay
        else:
                # 达到步数后，永远固定在 0.01
            alpha = min_alpha
        global_step += 1
        ...
        for (
            ...
            # if student_flag is False:
            # print("teacher obs", teacher_obs_batch)
            # jjj
            ...
            print("alpha: ", alpha, "guide_mix_ratio", self.policy.guide_mix_ratio)
